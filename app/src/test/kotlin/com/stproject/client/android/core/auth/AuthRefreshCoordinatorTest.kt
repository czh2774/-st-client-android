...
class AuthRefreshCoordinatorTest : BaseUnitTest() {
    ...
    @Test
    fun `refresh uses latest token when store already updated`() = runTest(mainDispatcherRule.dispatcher) {
    ...
    @Test
    fun `refresh calls service when token unchanged`() = runTest(mainDispatcherRule.dispatcher) {
    ...
    @Test
    fun `refresh returns null when service fails`() = runTest(mainDispatcherRule.dispatcher) {
        val store = FakeTokenStore(accessToken = "old-access", refreshToken = "rt-1")
        val service = FakeAuthService(store, result = null)
        val coordinator = AuthRefreshCoordinator(store, service)
