...
class AuthRefreshIntegrationTest {
                        ...
                        return when {
                            request.path?.startsWith("/api/v1/auth/login") == true -> {
                                ...
                                if (!body.contains("\"email\":\"user@example.com\"")) {
                                    return MockResponse()
                                        .setResponseCode(400)
                                        .setBody("""{"message":"missing email"}""")
                                }
                                if (!body.contains("\"password\":\"secret\"")) {
                                    return MockResponse()
                                        .setResponseCode(400)
                                        .setBody("""{"message":"missing password"}""")
                                }
                                if (request.getHeader("X-Auth-Client") != "android") {
                                    return MockResponse()
                                        .setResponseCode(400)
                                        .setBody("""{"message":"missing client header"}""")
                                }
                                MockResponse()
                                    .setResponseCode(200)
                                    .setBody(
    ...
    @Test
    fun `refreshes on 401 and retries once`() =
        runBlocking {
    ...
    @Test
    fun `clears tokens when refresh is unauthorized`() =
        runBlocking {
            server.dispatcher =
                object : Dispatcher() {
                    override fun dispatch(request: RecordedRequest): MockResponse {
    ...
    @Test
    fun `logout calls server and clears local state`() =
        runBlocking {
            server.dispatcher =
                object : Dispatcher() {
                    override fun dispatch(request: RecordedRequest): MockResponse {
                        return when {
                            request.path?.startsWith("/api/v1/auth/logout") == true -> {
                                ...
                                if (!body.contains("\"refreshToken\":\"rt-9\"")) {
                                    return MockResponse()
                                        .setResponseCode(400)
                                        .setBody("""{"message":"missing refresh"}""")
                                }
                                MockResponse()
                                    .setResponseCode(200)
                                    .setBody("""{"code":200,"data":{"message":"ok"}}""")
                            }
                            else -> MockResponse().setResponseCode(404)
                        }
                    }
    ...
    @Test
    fun `logout clears tokens and prevents refresh on 401`() =
        runBlocking {
