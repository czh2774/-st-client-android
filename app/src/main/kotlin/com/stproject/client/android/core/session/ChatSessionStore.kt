...
interface ChatSessionStore {
    ...
    fun getPrimaryMemberId(): String?

    fun setPrimaryMemberId(memberId: String?)
...
@Singleton
class SharedPreferencesChatSessionStore
    @Inject
    constructor(
        ...
        override fun getPrimaryMemberId(): String? = prefs.getString(KEY_PRIMARY_MEMBER_ID, null)

        override fun setPrimaryMemberId(memberId: String?) {
            val clean = memberId?.trim()?.takeIf { it.isNotEmpty() }
            prefs.edit().apply {
                if (clean == null) {
                    remove(KEY_PRIMARY_MEMBER_ID)
                } else {
                    putString(KEY_PRIMARY_MEMBER_ID, clean)
                }
            }.apply()
        }

        override fun getShareCode(): String? = prefs.getString(KEY_SHARE_CODE, null)

        override fun setShareCode(shareCode: String?) {
            val clean = shareCode?.trim()?.takeIf { it.isNotEmpty() }
            prefs.edit().apply {
                if (clean == null) {
                    remove(KEY_SHARE_CODE)
                } else {
                    putString(KEY_SHARE_CODE, clean)
                }
            }.apply()
        }
        ...
        private companion object {
            ...
            private const val KEY_PRIMARY_MEMBER_ID = "chat.primary_member_id"
            private const val KEY_SHARE_CODE = "chat.share_code"
