...
- Android is a partial implementation of the React client. Core chat + explore exist,
  but major product areas are missing (red packet, fan badges,
  creator tooling, world info, chat share, personas/masks/extensions, and rich settings).
...
- Message delete/undo/long-press tools: React DONE, Android PARTIAL (delete + delete-after on last assistant only)
- Local multi-session persistence: React DONE, Android PARTIAL (multi-session cache with eviction; restore UX pending)
- WorldInfo tooling: React DONE, Android MISSING
- Chat share page: React DONE, Android MISSING

### Creators / Assistants
- Creator list + characters: React DONE, Android DONE (basic)
- Creator assistant list/chat: React DONE, Android DONE (basic)
- Create/role form: React DONE, Android MISSING
...
- Model presets: React DONE, Android MISSING
...
- Comments, personas, masks, extensions
...
- No in-app disclosure/controls for UGC moderation beyond report/block.
...
- Add explicit restricted-content warnings and in-app reporting affordances.
...
- Room usage includes multi-session cache with eviction; restore on cold start
...
Missing:
- NSFW gating and age verification flows (unit + UI tests).
- Share-code resolve and direct navigation guardrails.
- Chat session lifecycle (create/open/restore/multi-session).
